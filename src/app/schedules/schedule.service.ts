...
export class ScheduleService {
  ...
  entity_url = environment.REST_API_URL + 'schedules';

  constructor(private _http: Http) {
  }

  getVets(): Observable<Schedule[]> {
    return this._http.get(this.entity_url)
      .map((response: Response) => <Schedule[]> response.json())
      .catch(this.handleError);
  }

  getVetById(vet_id: string): Observable<Schedule> {
    return this._http.get((this.entity_url + '/' + vet_id))
      .map((response: Response) => <Schedule> response.json())
      .catch(this.handleError);
  }

  updateVet(vet_id: string, vet: Schedule): Observable<Schedule> {
  ...
  addVet(vet: Schedule): Observable<Schedule> {
